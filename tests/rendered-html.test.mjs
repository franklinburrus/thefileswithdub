...
test("publishes a full-size X broadcast replay view with the official X player", async () => {
  ...
  assert.match(catalog, /GENERATED by `npm run broadcasts:sync` — do not hand-edit/);
  assert.match(catalog, /id: "1AKEmmrYnnnKL"/);
  const ids = [...catalog.matchAll(/^\s*\{\s*id: "([^"]+)"/gm)].map(m => m[1]);
  assert.ok(ids.length >= 11, `expected the sheet-synced catalog, got ${ids.length} entries`);
  assert.equal(new Set(ids).size, ids.length, "duplicate broadcast ids in catalog");
