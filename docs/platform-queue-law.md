...
**Law â approved by Frank 2026-09-22. Canonical source — supersedes copies elsewhere.**


Standing law: **every platform carries four things** â a production cadence, a
...
(AGENTS.md) still applies â empty slots are filled automatically with ready,
QA-passed content.

---

## 1. TikTok â @dubwiththefiles

**Production cadence:** Storm engine renders vertical 1080Ã1920 clips from
approved Titan selects. â¥5 clips/day, batched daily. Storm vertical spec is law
...
Floor: 15 clips (3 days). Below floor â same-day Storm production order.
Current (2026-09-22): 12 ready â 2.4 days. BELOW FLOOR. Storm owes â¥5/day
starting 2026-09-23.

**Posting cadence:** 5/day â 9:00 AM, 12:00 PM, 3:00 PM, 6:00 PM, 9:00 PM ET.
...
|---|---|---|---|
| CAPTCHA or verification challenge on sign-in | TikTok bot-flagged the session/IP | Stop. Ask Frank (preference: ask). Never solve blind | Main â Frank |
...
## 2. YouTube â DUB HERE (@TheFilesWithDub)
...
folder only ("Joe Budden" â Clips â per-episode folders). â¥1 clip/day,
...
Below floor â same-day production order to Dub:Video.
...
| Symptom | Diagnosis | Fix | Escalate to |
|---|---|---|---|
| Upload rejected / copyright claim | Content-ID match on JBP material | Review claim, dispute if fair-use clip; hold batch if systemic | Main â Frank |
...
## 3. Patreon â DUB / CANTDUBME

**Production cadence:** Dub:Video produces X Video Titan clips (1/day);
Dub:Audio produces X Audio Titan clips (1/day). Full episodes â Collections.

**Replenishment cadence:** Daily depth check. Floor: 7 videos + 7 audios.
Below floor â same-day production order to the owning room.
Current (2026-09-22): first post live; queue building.

**Posting cadence:** 2/day â 12:00 PM ET video, 6:00 PM ET audio. Free access
...
## 4. Apache â X + Instagram (Buffer API only)
...
â¥5 posts/day sustained (X + IG variants each).

**Replenishment cadence:** Daily depth check via Buffer API. Floor: 35 posts
(7 days). Below floor â backfill automatically per backfill law; production
order to Dub rooms same day.
Current (2026-09-22): 84 scheduled, covers through Sep 27 2:00 PM ET.
Gap Sep 27 2pâSep 30 11:30a (14 slots) blocked on 14 Sep-19 recuts due Sep 26.

**Posting cadence:** 5/day per the standing Apache slot grid. Method: Buffer
API ONLY â never the browser, never the provider session.
...
| Publish-failure email from Buffer | Often a false alarm â Buffer auto-retries | Re-read post status via API first; escalate only if still failed | Main |
...
## 5. Cherokee â X + Instagram (Buffer website only)

**Production cadence:** Cherokee production (Taxstone clips), X + IG pairs.
â¥12 posts/day sustained.

**Replenishment cadence:** Daily depth check via buffer.com website (never
the API â Cherokee owns the browser's Buffer session). Floor: 36 posts
(3 days). Below floor â production order to Cherokee room same day.
Current (2026-09-22): full through Sep 30 11:00 AM ET. Replenishment must
land by Sep 28.

**Posting cadence:** 12/day â X: 8:00a, 10:30a, 1:00p, 3:30p, 6:00p, 8:30p ET;
IG: 8:30a, 11:00a, 1:30p, 4:00p, 6:30p, 9:00p ET. Method: Buffer website via
the shared browser ONLY â never the API (single-key constraint).
...
## 6. Threads â @dubwiththefiles (tabled to 2026-09-23)
...
**Posting cadence:** 3/day â 9:00 AM, 1:00 PM, 6:00 PM ET.
...
## 7. Facebook â @dubwiththefiles (tabled to 2026-09-23)

**Production cadence:** Reuses approved full-bleed 1080Ã1920 IG variants.
2 posts/day.

**Replenishment cadence:** Daily depth check. Floor: 3 days (6 posts).

**Posting cadence:** 2/day â 11:00 AM, 6:00 PM ET.

**Problem-solving matrix:**

| Symptom | Diagnosis | Fix | Escalate to |
|---|---|---|---|
| Page not yet created | Tabled | Create 2026-09-23 â requires personal FB login | Frank |
...
1. **Daily depth check** (PM's morning verification pass): inventory Ã· burn
   rate per platform. Below floor â production order to the owner, same day.
2. **Backfill law** stays: empty slots fill automatically with ready,
   QA-passed content â never held hostage by reserved windows.
