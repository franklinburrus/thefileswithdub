...
const studioBookingLinks: Record<string, string> = {
  "1 hour": "https://calendly.com/dubwiththefiles/30-minute-meeting-clone",
  "2 hours": "https://calendly.com/dubwiththefiles/30-minute-meeting-clone-1",
  "3 hours": "https://calendly.com/dubwiththefiles/30-minute-meeting-clone-2",
  "4 hours": "https://calendly.com/dubwiththefiles/30-minute-meeting-clone-3",
};
...
export default function FilesPlatform({ page }: { page: Page }) {
  ...
  useEffect(() => {
    ...
    const closeOnEscape = (event: KeyboardEvent) => {
      ...
      const focusable = modalRef.current?.querySelectorAll<HTMLElement>("button:not([disabled]), a[href], iframe, input:not([disabled]), select:not([disabled]), textarea:not([disabled])");
  ...
  return <>
    ...
    {page === "home" && <>
      <section className="hero"><div className="hero-copy"><p className="kicker">INDEPENDENT MEDIA / QUEENS, NY</p><h1>THE CULTURE<br /><em>KEEPS RECEIPTS.</em></h1><p>Every clip, conversation, studio craft, and moment worth filing — all under one roof.</p><div className="button-row"><a className="solid" href={shopifyStorefront} target="_blank" rel="noreferrer">Shop the drop ↗</a><button className="outline" onClick={() => go("studio")}>Book the studio</button></div><small>Shop live inventory and checkout through the official Files Shopify store.</small></div><div className="hero-art"><img src="/dub-mark.svg" alt="The Files With Dub brand mark" width="1200" height="720" fetchPriority="high" /><div className="file-stamp">THE FILES<br />WITH DUB<br /><b>001</b></div></div></section>
...
function StudioBooking() { return <section className="panel light studio-page"><SectionTitle heading="h1" overline="THE STUDIO / SCHEDULING" title={<>Bring the story<br /><em>into focus.</em></>} copy="Choose a session type, then reserve available time through the official studio calendar." /><div className="studio-grid"><div className="studio-art"><picture><source srcSet="/dub-studio-mockup.webp" type="image/webp" /><img src="/dub-studio-mockup.png" alt="Podcast and video production studio mockup" width="1536" height="1024" /></picture><div>THE FILES STUDIO<br /><b>PRODUCTION-READY</b></div></div><div className="stack">{studioPackages.map(([name, copy]) => <article className="service-line" key={name}><h3>{name}</h3><p>{copy}</p><a className="service-link" href={studioBookingLinks[name]} target="_blank" rel="noreferrer">Book {name} ↗</a></article>)}<ScheduleDestination kind="studio" /></div></div></section>; }
function ConsultingFlow() { return <section className="panel consulting-page"><SectionTitle heading="h1" overline="DUB CONSULTING / SCHEDULING" title={<>Make the media<br /><em>make sense.</em></>} copy="Choose the working session that fits your goals, then select a time through the official consulting calendar." /><div className="service-grid">{consultingServices.map(([name, copy], index) => <article className="consult-card" key={name}><b>0{index + 1}</b><h3>{name}</h3><p>{copy}</p></article>)}</div><ScheduleDestination kind="consulting" /></section>; }
