...
export default function FilesPlatform({ page }: { page: Page }) {
  ...
  return <main>
    ...
    {page === "home" && <>
      ...
      <Newsletter onSubmit={event => { event.preventDefault(); setNotice("You’re on the dispatch demo. Connect an email provider to begin sending."); }} />
    </>}
...
function MediaGrid({ items, onSelect }: { items: MediaItem[]; onSelect: (item: MediaItem) => void }) { return <div className="media-grid">{items.map(item => <button className="media-card" key={item.id} onClick={() => onSelect(item)} aria-label={`Play ${item.title}`}><div><img src={item.image} alt="" /><span>▶</span><small>THE FILES / YOUTUBE</small></div><p>{item.date} · {item.runtime}</p><h3>{item.title}</h3><b>Play clip ↗</b></button>)}</div>; }
function Newsletter({ onSubmit }: { onSubmit: (event: FormEvent<HTMLFormElement>) => void }) { return <section className="newsletter"><div><p className="kicker">THE DISPATCH</p><h2>Stay in<br /><em>the files.</em></h2><p>Get the latest interviews, releases, events, and studio updates.</p></div><form onSubmit={onSubmit}><input required placeholder="First name" aria-label="First name" /><input required type="email" placeholder="Email address" aria-label="Email address" /><select aria-label="Content interests"><option>Content interests (optional)</option><option>Videos & interviews</option><option>Studio updates</option><option>Events & releases</option></select><label className="check"><input type="checkbox" required /> I agree to receive updates and accept the privacy policy.</label><button className="outline">Join the list ↗</button></form></section>; }
